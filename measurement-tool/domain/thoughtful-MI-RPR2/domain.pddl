...
;; move card from one column to another - create an empty column
	...
	(:action move-col-to-col-b
		...
		:precondition (and 
				...
				(canstack ?card ?newcard)
				(colspace ?cols)
				(or (colsuccessor ?ncols ?cols) (DUMMY-FLAG)))
		:effect (and 
			(on ?card ?newcard)
			(colspace ?ncols)
			(not (bottomcol ?card))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	(:action col-to-home
		...
		:precondition (and
			...
			(or (successor ?vcard ?vhomecard) (DUMMY-FLAG)))
		:effect (and
			(home ?card)
			(clear ?oldcard)
			(faceup ?oldcard)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	(:action tal-to-col
		...
		:precondition (and
			...
			(talonplayable ?card)
			(or (canstack ?card ?newcard) (DUMMY-FLAG)))
		:effect (and
			(clear ?card)
			(faceup ?card)
...
;; move card from talon to column - card is the only card in talon
	...
	(:action tal-to-col-d
		...
		:precondition (and
			...
			(bottomtalon ?card)
			(toptalon ?card)
			(or (talonplayable ?card) (DUMMY-FLAG)))
		:effect (and
			(clear ?card)
			(faceup ?card)
			(on ?card ?newcard)
...
;; move king from talon to column
	...
	(:action tal-to-col-e
		...
		:precondition (and
			...
			(talonplayable ?card)
			(colspace ?cols)
			(or (colsuccessor ?cols ?ncols) (DUMMY-FLAG)))
		:effect (and
			(clear ?card)
			(faceup ?card)
			(talonplayable ?oldcard)
