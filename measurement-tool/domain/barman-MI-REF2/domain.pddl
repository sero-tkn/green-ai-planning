(define (domain barman)
  ...
  (:action grasp
             ...
             :effect (and (DUMMY-FLAG) (not (DUMMY-FLAG)) (not (ontable ?c))
	     	     	  (not (handempty ?h))
			  (holding ?h ?c)))
  ...
  (:action leave
             ...
             :effect (and (DUMMY-FLAG) (not (DUMMY-FLAG)) (not (holding ?h ?c))
	     	     	  (handempty ?h)
			  (ontable ?c)))
  ...
  (:action fill-shot
           ...
           :precondition (and (holding ?h1 ?s)
	   		      ...
	   		      (dispenses ?d ?i)
                              (empty ?s)
			      (clean ?s))
           :effect (and (DUMMY-FLAG) (not (DUMMY-FLAG)) (not (empty ?s))
	   	   	(contains ?s ?i)
	   	   	(not (clean ?s))
			(used ?s ?i)))
  ...
  (:action empty-shot
           ...
           :precondition (and (holding ?h ?p)
                              (contains ?p ?b))
           :effect (and (DUMMY-FLAG) (not (DUMMY-FLAG)) (not (contains ?p ?b))
	   	   	(empty ?p)))
  ...
  (:action clean-shot
  	   :parameters (?s - shot ?b - beverage ?h1 ?h2 - hand)
           :precondition (and (holding ?h1 ?s)
			      ...
			      (empty ?s)
                              (used ?s ?b))
           :effect (and (DUMMY-FLAG) (not (DUMMY-FLAG)) (not (used ?s ?b))
	   	   	(clean ?s)))
           ...
           :precondition (and (holding ?h1 ?s)
			      (contains ?s ?i)
	   		      ...
	   		      (clean ?d)                              
                              (shaker-level ?d ?l)
                              (next ?l ?l1))
           :effect (and (DUMMY-FLAG) (not (DUMMY-FLAG)) (not (contains ?s ?i))
	   	   	(empty ?s)
			(contains ?d ?i)
                        (not (empty ?d))
			(not (clean ?d))
  ...
  (:action empty-shaker
           ...
           :effect (and (DUMMY-FLAG) (not (DUMMY-FLAG)) (not (shaked ?s))
  ...
  (:action clean-shaker
  	   :parameters (?h1 ?h2 - hand ?s - shaker)
           :precondition (and (holding ?h1 ?s)
                              (handempty ?h2)
                              (empty ?s))
           :effect (and (DUMMY-FLAG) (not (DUMMY-FLAG)) (clean ?s)))
  ...
  (:action shake
  	   :parameters (?b - cocktail ?d1 ?d2 - ingredient ?s - shaker ?h1 ?h2 - hand)
           :precondition (and (holding ?h1 ?s)
			      ...
			      (cocktail-part2 ?b ?d2)
			      (unshaked ?s))			      
           :effect (and (DUMMY-FLAG) (not (DUMMY-FLAG)) (not (unshaked ?s))
		        (not (contains ?s ?d1))
                        (not (contains ?s ?d2))
	   	   	(shaked ?s)
                        (contains ?s ?b)))
  ...
  (:action pour-shaker-to-shot
           ...
           :precondition (and (holding ?h ?s)
			      ...
			      (contains ?s ?b)
                              (shaker-level ?s ?l)
                              (next ?l1 ?l))
           :effect (and (DUMMY-FLAG) (not (DUMMY-FLAG)) (not (clean ?d))
	   	   	(not (empty ?d))
			(contains ?d ?b)
			(shaker-level ?s ?l1)
			(not (shaker-level ?s ?l))))
