...
;; move card from one column to another
	...
	(:action move-col-to-col
		...
		:precondition (and 
			...
			(on ?card ?oldcard))
		:effect(and
			(on ?card ?newcard) (not (on ?card ?newcard)) (on ?card ?newcard)
			(clear ?oldcard)
			(faceup ?oldcard)
			(not (on ?card ?oldcard))
			(not (clear ?newcard))))
...
;; send a king to an empty column
	...
	(:action move-col-to-col-c
		...
		:precondition (and
			...
			(on ?card ?oldcard))
		:effect (and
			(bottomcol ?card) (not (bottomcol ?card)) (bottomcol ?card)
			(clear ?oldcard)
			(faceup ?oldcard)
			(colspace ?ncols)
			(not (on ?card ?oldcard))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	(:action col-to-home
		...
		:effect (and
			(home ?card) (not (home ?card)) (home ?card)
			(clear ?oldcard)
			(faceup ?oldcard)
			(on ?card ?homecard)
			(not (on ?card ?oldcard))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	(:action tal-to-col
		...
		:precondition (and
			...
			(canstack ?card ?newcard))
		:effect (and
			(clear ?card) (not (clear ?card)) (clear ?card)
			(faceup ?card)
...
;; move card from talon to home
	...
	(:action tal-to-home
		...
		:precondition (and
			...
			(successor ?vcard ?vhomecard))
		:effect (and
			(talonplayable ?cardbelow) (not (talonplayable ?cardbelow)) (talonplayable ?cardbelow)
			(not (talonplayable ?card))
...
;; move card from talon to home - card is bottom card in talon
	...
	(:action tal-to-home-b
		...
		:precondition (and
			...
			(successor ?vcard ?vhomecard))
		:effect (and
			(bottomtalon ?cardabove) (not (bottomtalon ?cardabove)) (bottomtalon ?cardabove)
			(talonplayable ?cardabove)
			(home ?card)
			(not (bottomtalon ?card))
			(not (talonplayable ?card))
...
;; move card from talon to home - card is top card in talon
	...
	(:action tal-to-home-c
		...
		:precondition (and
			...
			(successor ?vcard ?vhomecard))
		:effect (and
			(toptalon ?cardbelow) (not (toptalon ?cardbelow)) (toptalon ?cardbelow)
...
;; move king from home to column
	...
	(:action home-to-col-a
		...
		:precondition (and
			...
			(colsuccessor ?cols ?ncols))
		:effect (and
			(home ?cardbelow) (not (home ?cardbelow)) (home ?cardbelow)
			(bottomcol ?card)
			(clear ?card)
			(faceup ?card)
			(colspace ?ncols)
...
;; turn deck
	...
	(:action turn-deck
		...
		:precondition (and
			...
			(ontalon ?c1 ?card))
		:effect (and
			(talonplayable ?c1) (not (talonplayable ?c1)) (talonplayable ?c1)
			(not (talonplayable ?card))))
