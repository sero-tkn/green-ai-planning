(define (domain barman)
  ...
  (:action grasp
             ...
             :effect (and (not (ontable ?c)) (ontable ?c) (not (ontable ?c))
	     	     	  (not (handempty ?h))
			  (holding ?h ?c)))
  ...
  (:action leave
             ...
             :effect (and (not (holding ?h ?c)) (holding ?h ?c) (not (holding ?h ?c))
	     	     	  (handempty ?h)
			  (ontable ?c)))
  ...
  (:action fill-shot
           ...
           :precondition (and (holding ?h1 ?s)
	   		      ...
	   		      (dispenses ?d ?i)
                              (empty ?s)
			      (clean ?s))
           :effect (and (not (empty ?s)) (empty ?s) (not (empty ?s))
	   	   	(contains ?s ?i)
	   	   	(not (clean ?s))
			(used ?s ?i)))
  ...
  (:action empty-shot
           ...
           :precondition (and (holding ?h ?p)
                              (contains ?p ?b))
           :effect (and (not (contains ?p ?b)) (contains ?p ?b) (not (contains ?p ?b))
	   	   	(empty ?p)))
  ...
  (:action clean-shot
  	   :parameters (?s - shot ?b - beverage ?h1 ?h2 - hand)
           :precondition (and (holding ?h1 ?s)
			      ...
			      (empty ?s)
                              (used ?s ?b))
           :effect (and (not (used ?s ?b)) (used ?s ?b) (not (used ?s ?b))
	   	   	(clean ?s)))
           ...
           :precondition (and (holding ?h1 ?s)
			      (contains ?s ?i)
	   		      ...
	   		      (clean ?d)                              
                              (shaker-level ?d ?l)
                              (next ?l ?l1))
           :effect (and (not (contains ?s ?i)) (contains ?s ?i) (not (contains ?s ?i))
	   	   	(empty ?s)
			(contains ?d ?i)
                        (not (empty ?d))
			(not (clean ?d))
  ...
  (:action empty-shaker
           ...
           :effect (and (not (shaked ?s)) (shaked ?s) (not (shaked ?s))
  ...
  (:action clean-shaker
  	   :parameters (?h1 ?h2 - hand ?s - shaker)
           :precondition (and (holding ?h1 ?s)
                              (handempty ?h2)
                              (empty ?s))
           :effect (and (clean ?s) (not (clean ?s)) (clean ?s)))
  ...
  (:action shake
  	   :parameters (?b - cocktail ?d1 ?d2 - ingredient ?s - shaker ?h1 ?h2 - hand)
           :precondition (and (holding ?h1 ?s)
			      ...
			      (cocktail-part2 ?b ?d2)
			      (unshaked ?s))			      
           :effect (and (not (unshaked ?s)) (unshaked ?s) (not (unshaked ?s))
		        (not (contains ?s ?d1))
                        (not (contains ?s ?d2))
	   	   	(shaked ?s)
                        (contains ?s ?b)))
  ...
  (:action pour-shaker-to-shot
           ...
           :precondition (and (holding ?h ?s)
			      ...
			      (contains ?s ?b)
                              (shaker-level ?s ?l)
                              (next ?l1 ?l))
           :effect (and (not (clean ?d)) (clean ?d) (not (clean ?d))
	   	   	(not (empty ?d))
			(contains ?d ?b)
			(shaker-level ?s ?l1)
			(not (shaker-level ?s ?l))))
