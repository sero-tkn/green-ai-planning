...
(:action dead-end-move-and-recharge
  :parameters (?rfrom - robot ?rto - robot
               ?from - location ?to - location
               ?fpre-move ?fpost-move - battery-level
               ?fpre-to ?fpost-to - battery-level
               ?fpre-from-after-move ?fpost-from-final - battery-level)
  :precondition
    (and
      (not (= ?rfrom ?rto))
      (not (stopped ?rfrom))
      (at ?rfrom ?from)
      (battery ?rfrom ?fpre-move)
      (BATTERY-PREDECESSOR ?fpost-move ?fpre-move)
      (or (CONNECTED ?from ?to) (CONNECTED ?to ?from))
      (at ?rto ?to)
      (battery ?rfrom ?fpre-from-after-move)
      (battery ?rto ?fpre-to)
      (BATTERY-PREDECESSOR ?fpost-from-final ?fpre-from-after-move)
  ...
  :effect
    (and
      (not (at ?rfrom ?from))
      ; REMOVED: (at ?rfrom ?to)
      (not (battery ?rfrom ?fpre-move))
      (battery ?rfrom ?fpost-move)
      (increase (total-cost) (move-cost))
      (not (battery ?rfrom ?fpre-from-after-move))
      (battery ?rfrom ?fpost-from-final)
