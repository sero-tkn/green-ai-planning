...
;; move card from one column to another
	...
	(:action move-col-to-col
		:parameters (?dummy - card ?card ?oldcard ?newcard - card)
		:precondition (and 
			(faceup ?card)
			(clear ?newcard)
...
;; move card from one column to another - create an empty column
	...
	(:action move-col-to-col-b
		:parameters (?dummy - card ?card ?newcard - card ?cols ?ncols - colnum)
		:precondition (and 
				(faceup ?card)
				(bottomcol ?card)
				(clear ?newcard)
...
;; send a king to an empty column
	...
	(:action move-col-to-col-c
		:parameters (?dummy - card ?card ?oldcard - card ?cols ?ncols - colnum)
		:precondition (and
			(faceup ?card)
			(king ?card)
			(colspace ?cols)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	(:action col-to-home
		:parameters (?dummy - card ?card ?oldcard - card ?suit - suittype ?vcard - num
...
;; Move card from column to home - create an empty column
	...
	(:action col-to-home-b
		:parameters (?dummy - card ?card ?homecard - card ?suit - suittype ?vcard - num
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	(:action tal-to-col
		:parameters (?dummy - card ?card ?oldcard ?newcard ?cardabove - card)
		:precondition (and
			(clear ?newcard)
...
;; move card from talon to column - card is bottom card in talon
	...
	(:action tal-to-col-b
		:parameters (?dummy - card ?card ?newcard ?cardabove - card)
		:precondition (and
			(clear ?newcard)
			(ontalon ?cardabove ?card)
			(talonplayable ?card)
...
;; move card from talon to column - card is top card in talon
	...
	(:action tal-to-col-c
		:parameters (?dummy - card ?card ?newcard ?oldcard - card)
		:precondition (and
			(clear ?newcard)
			(ontalon ?card ?oldcard)
			(canstack ?card ?newcard)
...
;; move card from talon to column - card is the only card in talon
	...
	(:action tal-to-col-d
		:parameters (?dummy - card ?card ?newcard - card)
		:precondition (and
			(clear ?newcard)
			(canstack ?card ?newcard)
			(bottomtalon ?card)
...
;; move king from talon to column
	...
	(:action tal-to-col-e
		:parameters (?dummy - card ?card ?oldcard ?cardabove - card ?cols ?ncols - colnum)
		:precondition (and
			(king ?card)
			(ontalon ?card ?oldcard)
			(ontalon ?cardabove ?card)
...
;; move king from talon to column - king is bottom card in talon
	...
	(:action tal-to-col-f
		:parameters (?dummy - card ?card ?cardabove - card ?cols ?ncols - colnum)
		:precondition (and
			(king ?card)
			(ontalon ?cardabove ?card)
			(talonplayable ?card)
...
;; move king from talon to column - card is the only card in talon
	...
	(:action tal-to-col-h
		:parameters (?dummy - card ?card - card ?cols ?ncols - colnum)
		:precondition (and
			(king ?card)
			(bottomtalon ?card)
			(toptalon ?card)
...
;; move card from talon to home
	...
	(:action tal-to-home
		:parameters (?dummy - card ?card ?cardabove ?cardbelow ?homecard - card ?cardsuit - suittype
			?vcard ?vhomecard - num)
		:precondition (and
			(talonplayable ?card)
			(ontalon ?cardabove ?card)
...
;; move card from talon to home - card is bottom card in talon
	...
	(:action tal-to-home-b
		:parameters (?dummy - card ?card ?cardabove ?homecard  - card ?cardsuit - suittype
			?vcard ?vhomecard - num)
		:precondition (and
			(talonplayable ?card)
			(ontalon ?cardabove ?card)
...
;; move card from talon to home - card is top card in talon
	...
	(:action tal-to-home-c
		:parameters (?dummy - card ?card ?cardbelow ?homecard - card ?cardsuit - suittype
			?vcard ?vhomecard - num)
		:precondition (and
			(ontalon ?card ?cardbelow)
			(talonplayable ?card)
...
;; move card from talon to home - card is the only card in talon
	...
	(:action tal-to-home-d
		:parameters (?dummy - card ?card ?homecard - card ?cardsuit - suittype
			?vcard ?vhomecard - num)
		:precondition (and
			(bottomtalon ?card)
			(toptalon ?card)
...
;; move card from home to column
	...
	(:action home-to-col
		:parameters (?dummy - card ?card ?cardbelow ?newcard - card ?cardsuit - suittype
...
;; move king from home to column
	...
	(:action home-to-col-a
		:parameters (?dummy - card ?card ?cardbelow - card ?cardsuit - suittype
...
;; turn deck
	...
	(:action turn-deck
		:parameters (?dummy - card ?card ?c1 - card)
		:precondition (and
			(talonplayable ?card)
			(ontalon ?c1 ?card))
		:effect (and
