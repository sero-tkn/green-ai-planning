...
(:action move
    ...
    :precondition
        (and
            ...
            (or (CONNECTED ?from ?to) (CONNECTED ?to ?from) (DUMMY-FLAG))
        )
    :effect
        (and
            (not (at ?r ?from))
...
(:action recharge
    ...
    :precondition
        (and
            ...
            (or (BATTERY-PREDECESSOR ?fpre-to ?fpost-to) (DUMMY-FLAG))
        )
    :effect
        (and
...
(:action verify-guard-config
    ...
    :precondition
        (and
            (forall (?l - location)
                (imply (GUARD-CONFIG ?c ?l) (or (guarded ?l) (DUMMY-FLAG)))
            )
        )
    :effect
        (and
