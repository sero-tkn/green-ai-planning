...
(:action move
    ...
    :effect
        (and
            (not (at ?r ?from)) (at ?r ?from) (not (at ?r ?from))
            (at ?r ?to)
            (not (battery ?r ?fpre))
            (battery ?r ?fpost)
...
(:action recharge
    ...
    :effect
        (and
            (not (battery ?rfrom ?fpre-from)) (battery ?rfrom ?fpre-from) (not (battery ?rfrom ?fpre-from))
...
(:action stop-and-guard
    ...
    :effect
        (and
            (stopped ?r) (not (stopped ?r)) (stopped ?r)
            (guarded ?l)
            (forall (?l2 - location)
...
(:action verify-guard-config
    ...
    :effect
        (and
            (forall (?r - robot) (not (stopped ?r)) (stopped ?r) (not (stopped ?r)))
