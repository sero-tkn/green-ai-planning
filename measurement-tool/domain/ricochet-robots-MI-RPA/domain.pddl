...
(:predicates
    ...
    (NEXT ?dummy - cell ?c - cell ?cnext - cell ?dir - direction)
    ;; moving from ?c in the direction ?dir is blocked
    (BLOCKED ?dummy - cell ?c - cell ?dir - direction)
    ;; Robot ?r is located in the cell ?c
    (at ?dummy - robot ?r - robot ?c - cell)
    ;; No robot is located in the cell ?c
    (free ?dummy - cell ?c - cell)
    ;; No robot is moving anywhere
    (nothing-is-moving)
    ;; Robot ?r is moving in the direction ?dir
    (is-moving ?dummy - robot ?r - robot ?dir - direction)
)
...
(:action step
    ...
    :precondition
        (and
            (is-moving ?r ?r ?dir)
            (at ?r ?r ?cfrom)
            (NEXT ?cfrom ?cfrom ?cto ?dir)
            (free ?cto ?cto)
            (not (BLOCKED ?cfrom ?cfrom ?dir))
        )
    :effect
        (and
            (not (at ?r ?r ?cfrom))
...
(:action stop-at-robot
    ...
    :precondition
        (and
            (is-moving ?r ?r ?dir)
            (at ?r ?r ?cat)
            (NEXT ?cat ?cat ?cnext ?dir)
            (not (free ?cnext ?cnext))
        )
    :effect
        (and
            (not (is-moving ?r ?r ?dir))
