...
#ifndef __BFWS_FORWARD_BACKWARD_H__
#define __BFWS_FORWARD_BACKWARD_H__
...
namespace aptk {
    ...
    namespace search {
        ...
        namespace backward{
            ...
            class Node {
            public:
                typedef aptk::backward::agnostic::Landmarks_Graph_Manager<bwd_Search_Model,Fwd_Search_Model>   Landmarks_Graph_Manager;
            ...
            public:
                ...
                typedef         aptk::backward::agnostic::Landmarks_Graph_Manager<bwd_Search_Problem,Fwd_Search_Problem>   Landmarks_Graph_Manager;
                typedef         aptk::backward::agnostic::Landmarks_Graph_Manager<bwd_Search_Problem,Fwd_Search_Problem>   Landmarks_Graph_Manager_Forward;
                typedef         aptk::backward::agnostic::Landmarks_Graph_Generator<bwd_Search_Problem, Fwd_Search_Problem>     Gen_Lms;
...
//                    m_novelty = new Abstract_Novelty( search_problem);
                ...
                void    set_relplan_bwd( Search_Node* n, aptk::backward::State* s ){
		...
		std::cout << "rel_plan size: "<< rel_plan.size() << " "<<std::flush;
                    ...
                    for(std::vector<Action_Idx>::iterator it_a = rel_plan.begin();
                        ...
                        const auto* a = this->problem_backward().task().actions()[*it_a];

                        //Add Conditional Effects
                        if( !a->ceff_vec().empty() ){
                            for( unsigned i = 0; i < a->ceff_vec().size(); i++ ){
                                auto* ce = a->ceff_vec()[i];
...
*/
                 ...
                 * @param s
                 */
                void    set_relplan_fwd( Search_Node* n, aptk::backward::State* s ){


                    std::vector<Action_Idx>	po;
                    std::vector<Action_Idx>	rel_plan;
                    unsigned h = 0;
                    aptk::backward::State* head_state;
                    if (n->parent()!=NULL){
		...
		std::cout << "rel_plan size: "<< rel_plan.size() << " "<<std::flush;
                    ...
                    for(std::vector<Action_Idx>::iterator it_a = rel_plan.begin();
                        it_a != rel_plan.end(); it_a++ ){
                        const auto* a = this->problem_forward().task().actions()[*it_a];
                 ...
                 * @param s
                 */
                void    set_relplan_edit( Search_Node* n, aptk::backward::State* s,Fluent_Vec unachived_goal  ){
...
//                m_second_h->eval_edit( *(candidate->state()));
                ...
                unsigned  rp_fl_achieved_bwd( Search_Node* n ){
                    ...
                    while( n->action()!= no_op && n != n_start ){
                        ...
                        const auto* a = this->problem_backward().task().actions()[ n->action() ];
...
*/
                ...
                unsigned  rp_fl_achieved_fwd( Search_Node* n ){
                    ...
                    while( n->action()!= no_op && n != n_start ){
                        ...
                        const auto* a = this->problem_forward().task().actions()[ n->action() ];
...
//                    aptk::backward::agnostic::Novelty<fwd_Search_Model,Search_Node>::eval( n, node_novelty );
//                    return node_novelty;
//                }
//                float   prune_bwd( Search_Node* n ){
//
//                    float node_novelty = infty;
//                    aptk::backward::agnostic::Novelty<bwd_Search_Model,Search_Node>::eval( n, node_novelty );
//                    return node_novelty;
//                }
...
//        }
                    ...
                    for (unsigned i = 0; i < app_set.size(); ++i ) {
                        ...
                        aptk::backward::State *succ = nullptr;
...
//            succ->print(std::cout);
                        ...
                        bool is_mutex = false;
                        const auto* a_ptr=this->problem_backward().task().actions()[a];
...
//                        }
                ...
                virtual std::pair<Search_Node*,Search_Node*> check_goal_in_closed_list(  Search_Node *head_forward,Search_Node *head_backward) {
                    ...
                    return std::make_pair<>(head_forward, head_backward);
                }
                ...
                virtual std::pair<Search_Node*,Search_Node*> check_goal_in_closed_bwd(  Search_Node *head_forward){
                    ...
                    return std::make_pair<>(head_forward, head_forward);
                }
                ...
                virtual std::pair<Search_Node*,Search_Node*> check_goal_in_closed_fwd(  Search_Node *head_backward){
                    ...
                    return std::make_pair<>(head_backward, head_backward);
                }
